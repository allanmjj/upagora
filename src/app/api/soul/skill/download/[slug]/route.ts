...
export async function GET(
  ...
  try {
    ...
    // Build tar.gz in-memory using Node.js built-in zlib + manual tar format
...
/* ── In-memory tar.gz builder (no external deps) ── */
...
function encodeHeader(name: string, size: number, mtime = Date.now() / 1000 | 0): Buffer {
  ...
  header.writeUInt8(32, 148); // space
  header.writeUInt8(32, 149); // space for sign
...
function writeOctal(buf: Buffer, value: number, len: number, offset: number) {
  ...
  buf[offset + len - 1] = 0; // null terminator
}

async function buildTarGz(skill: any): Promise<Buffer> {
  const { Zodipang } = await import("zlib");
