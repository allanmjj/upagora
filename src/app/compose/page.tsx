...
export default function ComposePage() {
  ...
  const submitDemand = async () => {
    setDError('')
    if (!dTitle.trim() || !dDesc.trim()) { setDError('标题和描述不能为空'); return }
    if (!me) { setDError('请先登录'); return }
    const budget = parseInt(dBudget) || 0
    if (budget > (me.credits || 0)) { setDError(`积分不足（当前 ${me.credits} 积分）`); return }
    setBusy(true)
    try {
      ...
      else { setDError(data.message || '发布失败') }
    } catch { setDError('网络错误，请重试') }
    finally { setBusy(false) }
  }
      ...
      {tab === 'post' ? (
        <div className='rounded-xl border border-zinc-800 bg-zinc-900/30 p-6'>
          <div className='flex items-start gap-3'>
            ...
            <div className='flex-1 space-y-4'>
              <textarea value={text} onChange={e => setText(e.target.value)} placeholder='分享你的想法...' className='w-full resize-none rounded-lg bg-transparent px-0 py-2 text-base text-zinc-50 placeholder:text-zinc-500 focus:outline-none' rows={6} maxLength={2000} />
              <div>
                <div className='flex items-center gap-2'><Hash className='h-4 w-4 text-zinc-500' /><input type='text' value={tagIn} onChange={e => setTagIn(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addTag())} placeholder='添加标签 (最多5个)...' className='w-full bg-transparent text-sm text-zinc-50 placeholder:text-zinc-500 focus:outline-none' disabled={tags.length >= 5} /></div>
          ...
          <div className='mt-4 flex items-center justify-between border-t border-zinc-800 pt-4'>
            <select value={vis} onChange={e => setVis(e.target.value as any)} className='rounded-lg border border-zinc-800 bg-zinc-900/50 px-3 py-1.5 text-sm text-zinc-400 focus:border-zinc-700 focus:outline-none'>
              <option value='public'>公开</option><option value='followers'>关注者</option><option value='private'>私密</option>
            </select>
            <div className='flex items-center gap-3'>
              ...
              <Button size='sm' className='bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white' onClick={submitPost} disabled={!text.trim() || busy}>
                {busy ? <Loader2 className='h-4 w-4 animate-spin' /> : <Send className='h-4 w-4' />}{busy ? ' 发布中...' : ' 发布'}
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className='rounded-xl border border-zinc-800 bg-zinc-900/30 p-6'>
          <div className='mb-4 flex items-center justify-between'>
            <h2 className='text-lg font-semibold text-zinc-50'>发布任务</h2>
            {me && <span className='text-sm text-zinc-400'>可用积分: <span className='text-indigo-400 font-medium'>{me.credits}</span></span>}
          </div>
          <div className='space-y-4'>
            <div>
              <label className='mb-1.5 block text-sm font-medium text-zinc-300'>任务标题</label>
              <input type='text' value={dTitle} onChange={e => setDTitle(e.target.value)} placeholder='一句话描述你的需求' className='w-full rounded-lg border border-zinc-700 bg-zinc-900/50 px-3 py-2 text-sm text-zinc-50 placeholder:text-zinc-600 focus:border-indigo-500 focus:outline-none' maxLength={120} />
            </div>
            <div>
              <label className='mb-1.5 block text-sm font-medium text-zinc-300'>详细描述</label>
              <textarea value={dDesc} onChange={e => setDDesc(e.target.value)} placeholder='详细描述任务内容、期望输出格式、交付标准等...' className='w-full resize-none rounded-lg border border-zinc-700 bg-zinc-900/50 px-3 py-2 text-sm text-zinc-50 placeholder:text-zinc-600 focus:border-indigo-500 focus:outline-none' rows={5} maxLength={5000} />
              <div className='mt-1 text-right text-xs text-zinc-600'>{dDesc.length}/5000</div>
            </div>
            <div className='grid grid-cols-2 gap-4'>
              <div>
                <label className='mb-1.5 block text-sm font-medium text-zinc-300 flex items-center gap-1.5'><DollarSign className='h-3.5 w-3.5 text-amber-400' />赏金积分</label>
                <input type='number' value={dBudget} onChange={e => setDBudget(e.target.value)} placeholder='0 (免费)' className='w-full rounded-lg border border-zinc-700 bg-zinc-900/50 px-3 py-2 text-sm text-zinc-50 placeholder:text-zinc-600 focus:border-indigo-500 focus:outline-none' min='0' />
                <p className='mt-1 text-xs text-zinc-600'>设为0则为免费任务</p>
              </div>
              <div>
                <label className='mb-1.5 block text-sm font-medium text-zinc-300 flex items-center gap-1.5'><Calendar className='h-3.5 w-3.5 text-blue-400' />截止日期</label>
                ...
                <p className='mt-1 text-xs text-zinc-600'>留空表示无期限</p>
              </div>
            </div>
            <div className='flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-900/30 p-3'>
              <div className='flex items-center gap-2'>
                ...
                <span className='text-sm text-zinc-300'>紧急任务</span>
              </div>
            ...
            <div>
              <label className='mb-1.5 block text-sm font-medium text-zinc-300'>标签</label>
              <div className='flex items-center gap-2'><Hash className='h-4 w-4 text-zinc-500' /><input type='text' value={dTagIn} onChange={e => setDTagIn(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addDTag())} placeholder='添加标签 (最多10个)...' className='w-full bg-transparent text-sm text-zinc-50 placeholder:text-zinc-500 focus:outline-none' disabled={dTags.length >= 10} /></div>
            ...
            <div className='flex items-center justify-between border-t border-zinc-800 pt-4'>
              <select value={dVis} onChange={e => setDVis(e.target.value as any)} className='rounded-lg border border-zinc-800 bg-zinc-900/50 px-3 py-1.5 text-sm text-zinc-400 focus:border-zinc-700 focus:outline-none'>
                <option value='public'>公开</option><option value='followers'>关注者可见</option><option value='private'>私密</option>
              </select>
              <Button size='sm' className='bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white' onClick={submitDemand} disabled={!dTitle.trim() || !dDesc.trim() || busy}>
                {busy ? <Loader2 className='h-4 w-4 animate-spin' /> : <Send className='h-4 w-4' />}{busy ? ' 发布中...' : ' 发布任务'}
              </Button>
            </div>
          </div>
