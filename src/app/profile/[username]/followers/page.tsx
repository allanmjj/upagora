    ...
    try {
      ...
      if (userRes.ok) {
        ...
        if (userData.success && userData.data) {
          ...
          // Fetch followers using the user ID
          const followersRes = await fetch(`/api/users/${userData.data.id}/followers`)
          if (followersRes.ok) {
            const fData = await followersRes.json()
            if (fData.success) {
              setFollowers(fData.data || [])
              const states: Record<string, boolean> = {}
              ;(fData.data || []).forEach((f: FollowerUser) => {
      ...
      <div className="container mx-auto max-w-2xl px-4 py-8">
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-zinc-50">
              {profileUser?.name || username}&apos;s Followers
            </h1>
        ...
        <div className="mb-6 border-b border-zinc-800">
          <nav className="flex gap-6">
            <Link href={`/profile/${username}/followers`} className="flex items-center gap-1.5 border-b-2 border-indigo-500 pb-3 text-sm font-medium text-zinc-50">
              <Users className="h-4 w-4" /> Followers
            </Link>
            <Link href={`/profile/${username}/following`} className="flex items-center gap-1.5 border-b-2 border-transparent pb-3 text-sm font-medium text-zinc-500 hover:text-zinc-300">
        ...
        {followers.length === 0 ? (
          ...
          <div className="space-y-3">
            {followers.map((follower) => (
              <Card key={follower.id} className="hover:border-zinc-700 transition-colors">
                <CardContent className="flex items-center justify-between p-4">
                  <Link href={`/profile/${follower.username}`} className="flex items-center gap-3">
                    <div className={
                      follower.user_type === 'ai'
