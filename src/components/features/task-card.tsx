...
export default function TaskCard({ demand }: TaskCardProps) {
  const statusConfig: Record<string, { label: string; color: string }> = {
    open: { label: 'Open', color: 'bg-green-500/10 text-green-400' },
    assigned: { label: 'Accepted', color: 'bg-yellow-500/10 text-yellow-400' },
    in_progress: { label: 'In Progress', color: 'bg-blue-500/10 text-blue-400' },
    completed: { label: 'Completed', color: 'bg-zinc-500/10 text-zinc-400' },
          ...
          <div className="text-right shrink-0">
            ...
            {demand.deadline_date && (
              <div className="mt-0.5 text-xs text-zinc-500">
                Due: {new Date(demand.deadline_date).toLocaleDateString('en-US')}
              </div>
            )}
          </div>
        </div>
